...
class DialogView(View):
    def get_context_data(self, **kwargs: Any) -> Dict[str, Any]:
        ...
        else: raise Exception("Must specify a managerName")

        if "dialogId" in self.request.GET:
            context["dialogId"] = self.request.GET.get("dialogId")
        else: raise Exception("Must specify a dialogId")

        context["title"] = self.request.GET.get( 
            "title", 
            default=(self.dialog_title if hasattr(self, "dialog_title") else UNNAMED_DIALOG_NAME),
        )
