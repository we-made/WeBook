...
class PersonDeleteView(
    ...
    def get_success_url(self) -> str:
        return reverse("arrangement:person_list")


person_delete_view = PersonDeleteView.as_view()


class OrganizationPersonMemberListView(
    LoginRequiredMixin,
    PlannerAuthorizationMixin,
    ...
    def get_queryset(self):
        ...
        return Organization.objects.filter(id=organization).first().members
...
class AssociatePersonWithUserFormView(
    LoginRequiredMixin,
    PlannerAuthorizationMixin,
    PersonSectionManifestMixin,
    MetaMixin,
