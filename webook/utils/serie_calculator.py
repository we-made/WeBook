...
def _day_seek(start_date: datetime, arbitrator: int, weekday: int) -> datetime:
    ...
    date decided by
     and weekday, in the given month of start_date.
...
def _pattern_strategy_every_arbitrary_date_of_month(cycle: _CycleInstruction) -> _Event:
    ...
    date = _day_seek(cycle.start_date, int(cycle.arbitrator), cycle.day_of_week - 1)
...
def calculate_serie(
    ...
    while (
        ...
        cycle_cursor += 1
    if scope.stop_within_date is not None:
        events = list(
            filter(lambda e: e.start.date() <= scope.stop_within_date.date(), events)
        )
    elif scope.instance_limit != 0:
